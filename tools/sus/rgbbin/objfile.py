...
class ObjectFile():
    ...
    def parse_header(self):
        ...
        if signature not in (b"RGB5", b"RGB6", b"RGB9"):
            raise ObjectParseError("not a valid RGBASM 5/6 object file")
        self.revision = self.read_dword()
        self.symbol_count = self.read_dword()
        self.section_count = self.read_dword()
        self.node_count = self.read_dword()
        self.state = ParseState.HEADER_PARSED


    def parse_source_info(self):
        for _ in range(0, self.node_count):
            parent_id = self.read_dword()
            parent_line = self.read_dword()
            type = self.read_byte()
            if type != 0:
                name = self.read_string()
            else:
                depth = self.read_dword()
    ...
    def parse_symbols(self):
        ...
        for i in range(0, self.symbol_count):
            ...
            if symbol_type == 1:
                print("WARNING: undefined reference to %s" % symbol_name)
                continue
                # an external symbol in single-file linking means undefined symbol
                # raise ObjectParseError()

            node_id = self.read_dword()
            line_no = self.read_dword()
            symbol_sectid = self.read_dword()
            symbol_value = self.read_dword()

            logger.verbose("symbol %s defined in %d:%d (id=%i, sectid=%i) = $%.4x"
                % (symbol_name, node_id, line_no, i, symbol_sectid, symbol_value))
            self.symbols.append({
                'name': symbol_name,
    ...
    def parse_sections(self):
        ...
        for i in range(0, self.section_count):
            ...
            section_address = self.read_dword()
            section_bank = self.read_dword()
            section_align = self.read_byte()
            section_align_ofs = self.read_dword()
            ...
            for j in range(0, section_patch_count):
                patch_node_id = self.read_dword()
                patch_lineno = self.read_dword()
                patch_offset = self.read_dword()
                patch_pcsectionid = self.read_dword()
                patch_pcoffset = self.read_dword()
                ...
                section_patches.append({
                    "source": "%s:%i" % (patch_node_id, patch_lineno),
                    "offset": patch_offset,
                    "type": patch_type,
                    "pcsectionid": patch_pcsectionid,
                    "pcoffset": patch_pcoffset,
                    "rpn": patch_rpn
                })
            self.sections.append({
                "name": section_name,
                "origin": section_address,
                "data": bytearray(section_data),
                "patches": section_patches,
                "sectid": len(self.sections)
            })
            print("section %s of size %i at org $%.4x with %i patches"
                % (section_name, section_size, section_address, section_patch_count))
